...
impl TxqHandle {
    ...
    fn queue_setup(&mut self) -> Result<(), String> {
        let rc = unsafe {
            rte_eth_tx_queue_setup(
                ...
                &self._tx_conf.tx_conf as *const _,
            )
        };
        if rc < 0 {
    ...
    fn queue_setup(&mut self) -> Result<(), String> {
        let rc = unsafe {
            rte_eth_rx_queue_setup(
                ...
                self.desc_num,
                NumaSocketId::NumaSocketIdPort(self.port_id).to_socket_id() as _,
                &self._rx_conf.rx_conf as *const _,
...
impl Port {
    ...
    pub fn config_rxqs(&mut self, desc_num: u16, mempool: DpdkMempool) -> Result<(), String> {
        ...
        for qid in 0..q_num {
            let mut rxq_handle = RxqHandle::new(
                self.handle.clone(),
                self.port_id, qid,
                desc_num,
                mempool.clone()
            );
            rxq_handle.queue_setup().unwrap();
            rxqs.push(rxq_handle);
        }
        self.rxqs = Some(rxqs);
        Ok(())
    }
    ...
    pub fn config_txqs(&mut self, desc_num: u16) -> Result<(), String> {
        ...
        for qid in 0..q_num {
            ...
            txq_handle.queue_setup().unwrap();
            txqs.push(txq_handle);
        }
        self.txqs = Some(txqs);
